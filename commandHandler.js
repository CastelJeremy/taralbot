import { createAudioPlayer, createAudioResource, demuxProbe, joinVoiceChannel } from '@discordjs/voice';
import yt from './ytHandler.js';

async function play (bot, message) {
	if (!message.member.voice.channelId) {
		message.channel.send('Il faut être connecté à un salon vocal pour que je puisse jouer 🐷');
		return;
	}

	if (bot.connection && bot.connection.joinConfig.guildId == message.guild.id && bot.connection.joinConfig.channelId != message.member.voice.channel.id) {
	...
	try {
		...
		if (null == regexFull.exec(videoUrl) && null == regexShort.exec(videoUrl))
		...
		if (e.message == 'No result for the specified string')
			...
			message.channel.send('Oula 🐷, j\' ai eu un petit soucis. Tu veux bien réessayer ?');
			throw e;
		}
	}
}
